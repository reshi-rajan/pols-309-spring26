...
model1 <- lm(FEDFUNDS ~ inflation + election + lag_FEDFUNDS + democrat, data = fed_reserve)
...
model2 <- lm(log(FEDFUNDS)~ inflation + election + log(lag_FEDFUNDS) + democrat, data = fed_reserve)

summary(model2)

## Your Turn: Interpretation:

# A one-unit increase in inflation is associated with a [0.01659989 x100]
#     percent [increase] in Federal Reserve Interest Rate, all else equal

### Linear-Log

model3 <- lm(FEDFUNDS ~ log(inflation) + lag_FEDFUNDS + election + democrat, data = fed_reserve)

summary(model3)

## Your Turn: Interpretation: 

# A one-percent increase in inflation (logged) is associated with a [0.236711/100]
##  [increase] in the Fed's interest rate 

### Log-Log

model4 <- lm(log(FEDFUNDS) ~ log(inflation) + log(lag_FEDFUNDS) + democrat + election, data = fed_reserve)

summary(model4)

## Your Turn: Interpretation:

# A one-percent increase in inflation is associated with a [0.0052754] percent
##      [decrease] in Fed's Interest Rate
...
model5 <- lm(inc_2p_share ~ inc_pos + st_uemp + ch_qual, data = senate)
...
AME <- slopes(model6,
              variables = "inc_pos",
              newdata=datagrid(inc_pos=seq(-.5, .5, by=.1)))
