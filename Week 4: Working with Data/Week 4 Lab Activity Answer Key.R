...
## How could we generate a new dataframe that 
## gets us the average number of gears, minimum number of gears,
## variance of gears, and maximum number of gears per cylinder type?

data <- mtcars %>%
  reframe(
    min_gears = min(gear),
    mean_gears = mean(gear),
    var_gears = var(gear),
    max_gears = max(gear),
    .by = cyl
  )
